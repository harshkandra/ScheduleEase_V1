...
export default function BookAppointmentModal({
  ...
  useEffect(() => {
    ...
    if (preselectedSlot) {
      setForm((s) => ({
        ...
        time: preselectedSlot.timeStart,
      }));
    }
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await axios.get(
          `http://localhost:5000/api/slots?date=${form.date}`,
  ...
  const handleSubmit = async (e) => {
    ...
    const selectedSlot = availableSlots.find(
      (s) => s.timeStart === form.time && s.date === form.date
    );
    if (!selectedSlot) {
    ...
    payload.append("slotId", selectedSlot._id);
    payload.append("title", form.purpose);
    payload.append("description", `${form.meetingType}`);
    if (form.docs) payload.append("agendaFile", form.docs);
        ...
        >
          ...
          <div>
            ...
            <input
              ...
              accept=".pdf,.doc,.docx"
              onChange={(e) =>
                setForm((s) => ({ ...s, docs: e.target.files[0] }))
              }
